// bubble-renderer-full.js — Telegram 2026 FULL Bubble Renderer + Joiner Simulator Integration
(function(){
  'use strict';
  ...
  function init(){
    ...
    function applyStickerEffects(stickerEl){
      if(!stickerEl) return;
      const avatars = Array.from(stickerEl.querySelectorAll('img.join-avatar, .join-more'));
      avatars.forEach(el=>{
        const offset = Math.random()*2000, amplitude = 2 + Math.random()*2, period = 3000 + Math.random()*2000;
        let start = performance.now() + offset;
        function floatLoop(time){
          const t = (time-start)/period;
          if(!el.matches(':hover')) el.style.transform = `translateY(${Math.sin(t*2*Math.PI)*amplitude}px)`;
          requestAnimationFrame(floatLoop);
        }
        requestAnimationFrame(floatLoop);
        el.addEventListener('mouseenter', ()=>{
          el.style.transition = 'transform 0.3s ease-out';
          el.style.transform += ' scale(1.18) translateY(-4px)';
          el.style.zIndex = 999;
        });
    ...
    if(window.joiner){
      ...
      if(Array.isArray(window.joiner.preGenPool) && window.joiner.preGenPool.length){
        window.joiner.preGenPool.forEach(joiner=>{
          const text = window.WELCOME_TEXT_POOL?.[Math.floor(Math.random()*window.WELCOME_TEXT_POOL.length)] || 'Hi!';
          appendMessage(joiner, text, { type:'incoming' });
        });
        const initialBurst = window.joiner.preGenPool.slice(0, window.joiner.config?.initialBurstPreview || 3);
        if(initialBurst.length){
          const stickerEl = window.joiner.createJoinStickerElement?.(initialBurst);
          if(stickerEl && container){
            container.appendChild(stickerEl);
            applyStickerEffects(stickerEl);
          }
        }
      }
    }
    ...
    console.log('bubble-renderer FULL integrated with joiner-simulator & floating pre-gen effects');
  }
